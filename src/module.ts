...
export const plugin = new PanelPlugin<AstroOptions>(AstroPanel).setPanelOptions((builder) => {
  ...
  builder.addBooleanSwitch({
    ...
    defaultValue: false,
    description:
      'Hides the Astro panel entirely, allowing for extra dashboard space while still retaining the styling the panel provides. If enabled, you may need to edit the JSON model to see the panel options again.',
  });
  builder.addSelect({
    path: 'classification',
    name: 'Classification',
